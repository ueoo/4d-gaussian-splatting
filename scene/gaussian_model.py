...
from utils.sh_utils import RGB2SH, sh_channels_4d
...
class GaussianModel:
    ...
    def get_current_covariance_and_mean_offset(self, scaling_modifier=1, timestamp=0.0):
        return self.covariance_activation(
            self.get_scaling_xyzt, scaling_modifier, self._rotation, self._rotation_r, dt=timestamp - self.get_t
        )
    ...
    def training_setup(self, training_args):
        ...
        if self.gaussian_dim == 4:  # TODO: tune time_lr_scale
            ...
            l.append(
                {"params": [self._t], "lr": training_args.position_t_lr_init * self.spatial_lr_scale, "name": "t"}
            )
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, grads_t, grad_t_threshold, N=2):
        ...
        if not self.rot_4d:
            ...
            samples = torch.normal(mean=means, std=stds)
            rots = build_rotation_4d(self._rotation[selected_pts_mask], self._rotation_r[selected_pts_mask]).repeat(
                N, 1, 1
            )
